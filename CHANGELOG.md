...
- Settings/Plugins: added a Plugins page for managing ax-code plugins, with npm update checks and user/project scopes (thanks to @Quat3rnion).
...
- Chat: added reusable snippets with `#` autocomplete in the composer and a Snippets settings page for global and project snippets with [ax-code-snippets](https://github.com/JosXa/ax-code-snippets) plugin compatibility.
...
- Updates/PWA: AX Code update and PWA install prompts can now be dismissed without reappearing repeatedly (thanks to @robertoberto).
...
- Skills: installed skills now match AX Code's own skill list more closely.
...
- Updates/Usage: added a setting to disable AX Code update notifications, and quota reset times now display in your local timezone.
...
- Updates/AX Code: added in-app AX Code update checks and upgrade actions.
...
- Startup/Reliability: configured AX Code CLI paths are now validated before managed startup, with clearer errors for missing, non-executable, or app-bundle paths.
...
- Reliability/Streaming: reconnects now recover immediately after OS wake-from-sleep, long agent sessions avoid streaming hangs, concurrent sessions sharing the same provider are throttled more safely, and model metadata refreshes after AX Code restarts (thanks to @jwcrystal, @pasta-paul, @Yabuku-xD).
- Onboarding/Updates/Mobile: added AX Code CLI auto-detection during onboarding, cross-checks update prompts against npm, and improved iPad/tablet controls for fewer false update notices and smooth touch use (thanks to @IslamNofl).
...
- Reliability/Startup: hardened managed AX Code startup, preserved shell PATH reliably, ignored stale downgrade update prompts, and improved stream/proxy recovery with heartbeat support.
...
- Settings/MCP: improved MCP auth flow with better remote-config support and clearer diagnostics, and aligned config resolution with AX Code behavior (thanks to @daveotero, @cyan).
...
- Usage: added Zhipu AI Coding Plan tracking and restored model-variant compatibility with older AX Code runtimes (thanks to @cainiao1992, @Chi-square-test).
...
- Reliability/Proxy: hardened AX Code proxy header handling (including identity-encoding normalization, compression-header cleanup, hop-by-hop response-header stripping) and suppressed expected SSE close noise.
...
- Reliability: improved Windows compatibility across git status checks, AX Code startup, path normalization, and session merge behavior (thanks to @mmereu).
...
- Web: added `AX_CODE_HOST` support (thanks to @colinmollenhour).
...
- Reliability: refactored AX Code config/auth integration into domain modules for steady provider auth and command loading flows (thanks to @nelsonPires5).
...
- Skills: discovery now uses AX Code API as the source of truth with safer fallback scanning.
- Skills: upgraded editing/install UX with better code editing, syntax-aware related files, and clearer location targeting across user/project .ax-code and .agents scopes.
...
- Sessions: project icon and color customizations now persist reliably across restarts.
  **- Reliability: managed local AX Code runtimes now use rotated secure auth and tighter lifecycle control across runtimes.**
...
- Reliability: startup now auto-detects and safely connects to an existing AX Code server.
...
- Reliability: fixed AX Code auth pass-through and proxy env handling to reduce intermittent connection/auth issues (thanks to @gsxdsm).

## [1.6.5] - 2026-02-6

- Settings: added an AX Code CLI path override.
...
- Reliability: improved AX Code binary resolution and HOME-path handling across runtimes for steady local startup.
...
- Worktrees: migrated to the AX Code SDK worktree implementation; sessions in worktrees are now completely isolated.
...
- Reliability: connect to external AX Code servers without auto-start and fixed subagent crashes (thanks to @TaylorBeeston).
...
- Reliability: improved AX Code process cleanup to reduce orphaned servers.

## [1.5.1] - 2026-01-16

- Desktop: fixed orphaned AX Code processes not being cleaned up on restart or exit.
- AX Code: fixed a crash when reloading configuration.
...
- Web/VSCode: fixed orphaned AX Code processes not being cleaned up on restart or exit.
...
- Settings: consolidated Git settings and added ax-code zen model selection for commit generation (thanks to @nelsonPires5).
...
- VSCode/Web: switched AX Code CLI management to the SDK.
...
- Added permission prompt UI for tools configured with "ask" in ax-code.json, showing requested patterns and "Always Allow" options (thanks to @aptdnfapt).
...
- Migrated to AX Code SDK v2 with improved API types and streaming.
...
- Added skills management to settings with the ability to create, edit, and delete skills (make sure you have the latest AX Code version for skills support).
...
- Improved dev server HMR by reusing a healthy AX Code process to avoid zombie instances.
...
- Added Nushell support for AX Code CLI operations.
...
- Updated AX Code SDK to 1.0.185 across all app versions.
- VS Code extension: fixed startup, more reliable AX Code CLI/API management, and stabilized API proxying/streaming.
...
- Updated AX Code SDK to the latest version.
...
- Introduced AX Code agent chat experience with section-based navigation, theming, and session persistence.
