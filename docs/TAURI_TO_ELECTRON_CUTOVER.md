...
3. **Electron `appId` is `dev.openchamber.desktop`** (check
   ...
   `ai.ax-code.openchamber`. These differ intentionally — it means macOS
...
`ai.ax-code.openchamber` with `dev.openchamber.desktop` at the same `.app`
...
The `openchamber://` protocol was registered for `ai.ax-code.openchamber`.
