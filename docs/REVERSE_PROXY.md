...
Use this guide when running AX Code App behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm AX Code App works directly first.
...
- Disable gzip on the proxy if AX Code App is already compressing responses.
...
- AX Code App reachable directly on LAN
...
- AX Code App compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by AX Code App, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
