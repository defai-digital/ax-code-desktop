...
1. Confirm AX Code Desktop works directly first.
...
- Disable gzip on the proxy if AX Code Desktop is already compressing responses.
...
- AX Code Desktop reachable directly on LAN
...
- AX Code Desktop compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by AX Code Desktop, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
