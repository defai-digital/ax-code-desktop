...
const DEFAULT_AX_CODE_ROOT = path.join(os.homedir(), 'projects', 'ax-code');
const DEFAULT_DESKTOP_SOURCE_DIR = path.join(DEFAULT_AX_CODE_ROOT, 'packages', 'ui', 'src', 'theme', 'themes');
const DEFAULT_CONTEXT_SOURCE_DIR = path.join(
  DEFAULT_AX_CODE_ROOT,
  'packages',
  'ax-code',
  'src',
  'cli',
...
const usage = () => {
  console.error(
    [
      'Usage: tsx scripts/port-ax-code-theme.ts [options] <theme-id|path> [...more]',
      '',
      'Options:',
      '  --ax-code-root <path>  AX Code repo root (default: ~/projects/ax-code)',
      ...
      '  --list                  List available AX Code themes',
    ].join('\n'),
  );
};
...
function parseArgs(argv: string[]): ParsedArgs {
  const args: ParsedArgs = {
    axCodeRoot: DEFAULT_AX_CODE_ROOT,
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--ax-code-root') {
      ...
      if (!value) throw new Error('Missing value for --ax-code-root');
      args.axCodeRoot = expandHome(value);
      index += 1;
      continue;
    }
...
async function listThemes(axCodeRoot: string): Promise<void> {
  const sourceDirs = [
    path.join(axCodeRoot, 'packages', 'ui', 'src', 'theme', 'themes'),
    path.join(axCodeRoot, 'packages', 'ax-code', 'src', 'cli', 'cmd', 'tui', 'context', 'theme'),
  ];
...
async function resolveThemeSpec(spec: string, axCodeRoot: string): Promise<string> {
...
async function loadResolver(axCodeRoot: string): Promise<(variant: ThemeVariant, isDark: boolean) => ResolvedTheme> {
  const resolvePath = path.join(axCodeRoot, 'packages', 'ui', 'src', 'theme', 'resolve.ts');
  if (!(await exists(resolvePath))) {
    throw new Error(`AX Code resolver not found: ${resolvePath}`);
  }
...
function buildTheme(
  ...
  return {
    metadata: {
      ...
      description: `Ported from AX Code ${desktopTheme.name} theme (${mode} variant)`,
      author: 'AX Code',
      version: '1.0.0',
      variant: mode,
      tags: [mode, 'ax-code', 'ported', desktopTheme.id],
    },
    colors: {
      primary: {
        base: primaryBase,
...
async function main(): Promise<void> {
  ...
  if (args.list) {
    await listThemes(args.axCodeRoot);
    return;
  }
  ...
  const resolveThemeVariant = await loadResolver(args.axCodeRoot);
  const generated: Array<{ fileName: string; theme: OpenChamberTheme }> = [];

  for (const spec of args.specs) {
    const themePath = await resolveThemeSpec(spec, args.axCodeRoot);
