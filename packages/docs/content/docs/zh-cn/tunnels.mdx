---
title: 隧道
description: 安全地将 AX Code App 开放给远程和移动访问。
---

# 隧道

隧道是指向你的 AX Code App 的一个公开链接，让你可以从手机或另一个网络访问它。使用 `openchamber tunnel` 为正在运行的实例创建一个隧道。

## 前置要求

AX Code App 会在你的机器上启动隧道提供商的 CLI。请先安装你想使用的提供商：
...
隧道建立后，`status` 会显示一个公开 URL。打开它，或扫描二维码，即可从任何地方访问 AX Code App。

默认情况下，AX Code App 会在交互式 TTY 会话中输出二维码。使用 `--qr` 强制输出二维码，或使用 `--no-qr` 将其禁用。
...
- [PWA 与移动访问](/zh-cn/mobile/) — 从手机访问 AX Code App
