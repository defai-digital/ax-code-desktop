---
title: Túneis
description: Exponha o AX Code App com segurança para acesso remoto e móvel.
---

# Túneis

Um túnel é um link público para o seu AX Code App, para que você possa acessá-lo pelo celular ou de outra rede. Use `openchamber tunnel` para criar um para uma instância em execução.
...
Quando o túnel estiver no ar, o `status` mostra uma URL pública. Abra-a, ou escaneie o QR code, para acessar o AX Code App de qualquer lugar.

Por padrão, o AX Code App imprime um QR code em sessões TTY interativas. Use `--qr` para forçar a saída do QR, ou `--no-qr` para desativá-la.
...
- [PWA e Acesso Móvel](/pt-br/mobile/) — acesse o AX Code App pelo celular
