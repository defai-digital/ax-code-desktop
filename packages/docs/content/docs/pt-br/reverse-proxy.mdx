---
title: Proxy reverso
description: Configure o AX Code App corretamente atrás do Nginx, Nginx Proxy Manager ou outro proxy reverso.
---

# Proxy reverso

Use esta página se você executa o AX Code App atrás do Nginx, Nginx Proxy Manager, Caddy, Cloudflare ou outro proxy reverso.

## Antes de colocar um proxy

1. Primeiro confirme que o AX Code App funciona diretamente.
...
- Desative o gzip no proxy se o AX Code App já comprime as respostas.
...
- AX Code App acessível diretamente na LAN
...
- O AX Code App comprime as respostas HTTP com gzip (limite de 1 KB).
...
As rotas de streaming SSE são excluídas da compressão pelo AX Code App, mas a CDN ainda pode armazená-las em buffer. Consulte a documentação da sua CDN sobre como desativar o buffer nas rotas SSE.
