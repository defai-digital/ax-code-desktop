---
title: Túneles
description: Expón AX Code App de forma segura para acceso remoto y móvil.
---

# Túneles

Un túnel es un enlace público a tu AX Code App, para que puedas acceder a él desde tu teléfono u otra red. Usa `openchamber tunnel` para crear uno para una instancia en marcha.
...
Cuando el túnel está activo, `status` muestra una URL pública. Ábrela, o escanea el código QR, para acceder a AX Code App desde cualquier lugar.

Por defecto, AX Code App imprime un código QR en sesiones TTY interactivas. Usa `--qr` para forzar la salida del QR, o `--no-qr` para desactivarla.
...
- [PWA y acceso móvil](/es/mobile/) — accede a AX Code App desde tu teléfono
