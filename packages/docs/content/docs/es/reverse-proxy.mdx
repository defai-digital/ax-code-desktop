---
title: Proxy inverso
description: Configura AX Code App correctamente detrás de Nginx, Nginx Proxy Manager u otro proxy inverso.
---

# Proxy inverso

Usa esta página si ejecutas AX Code App detrás de Nginx, Nginx Proxy Manager, Caddy, Cloudflare u otro proxy inverso.

## Antes de ponerle un proxy

1. Confirma primero que AX Code App funciona directamente.
...
- Desactiva gzip en el proxy si AX Code App ya comprime las respuestas.
...
- AX Code App accesible directamente en la LAN
...
- AX Code App comprime las respuestas HTTP con gzip (umbral de 1 KB).
...
Las rutas de streaming SSE están excluidas de la compresión por AX Code App, pero la CDN aún puede almacenarlas en búfer. Consulta la documentación de tu CDN para saber cómo desactivar el búfer en las rutas SSE.
