---
title: Reverse proxy
description: Skonfiguruj AX Code App poprawnie za Nginx, Nginx Proxy Manager lub innym reverse proxy.
---

# Reverse proxy

Skorzystaj z tej strony, jeśli uruchamiasz AX Code App za Nginx, Nginx Proxy Manager, Caddy, Cloudflare lub innym reverse proxy.

## Zanim ustawisz proxy

1. Najpierw potwierdź, że AX Code App działa bezpośrednio.
...
- Wyłącz gzip na proxy, jeśli AX Code App już kompresuje odpowiedzi.
...
- AX Code App dostępny bezpośrednio w LAN
...
- AX Code App kompresuje odpowiedzi HTTP za pomocą gzip (próg 1 KB).
...
Trasy strumieniowania SSE są wyłączone z kompresji przez AX Code App, ale CDN może je nadal buforować. Sprawdź dokumentację swojego CDN, jak wyłączyć buforowanie na ścieżkach SSE.
