---
title: Tunnels
description: Expose AX Code App safely for remote and mobile access.
---

# Tunnels

A tunnel is a public link to your AX Code App, so you can reach it from your phone or another network. Use `openchamber tunnel` to create one for a running instance.

## Prerequisites

AX Code App starts the tunnel provider CLI on your machine. Install the provider you want to use first:
...
When the tunnel is up, `status` shows a public URL. Open it, or scan the QR code, to reach AX Code App from anywhere.

By default, AX Code App prints a QR code in interactive TTY sessions. Use `--qr` to force QR output, or `--no-qr` to disable it.
...
- one active tunnel per AX Code App instance (port)
...
- [PWA & Mobile](/mobile/) — reach AX Code App from your phone
