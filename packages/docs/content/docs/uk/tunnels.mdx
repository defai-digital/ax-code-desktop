---
title: Тунелі
description: Безпечно відкрийте AX Code App для віддаленого та мобільного доступу.
---

# Тунелі

Тунель — це публічне посилання на ваш AX Code App, щоб дістатися до нього з телефона чи з іншої мережі. Скористайтеся `openchamber tunnel`, щоб створити його для запущеного інстансу.

## Передумови

AX Code App запускає CLI провайдера тунелю на вашій машині. Спочатку встановіть провайдера, який хочете використовувати:
...
Коли тунель піднято, `status` показує публічний URL. Відкрийте його або відскануйте QR-код, щоб дістатися до AX Code App звідусіль.

Типово AX Code App виводить QR-код в інтерактивних TTY-сесіях. Використовуйте `--qr`, щоб примусово вивести QR, або `--no-qr`, щоб вимкнути його.
...
- один активний тунель на інстанс AX Code App (порт)
...
- [PWA та мобільний доступ](/uk/mobile/) — відкривайте AX Code App з телефона
