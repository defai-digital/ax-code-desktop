...
Every model can only hold so much of a conversation at once — its context. AX Code Desktop shows how full that is, so you can tell when a session is getting close to the limit and a reply might start dropping older detail.
...
Hover it or open the details popover to see exact token counts.
