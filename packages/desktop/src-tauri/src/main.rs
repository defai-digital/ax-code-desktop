...
async fn wait_for_local_ax_code_ready_with(
    url: &str,
    timeout: Duration,
...
async fn spawn_local_server(app: &tauri::AppHandle) -> Result<String> {
    ...
    if let Some(val) = ax_code_binary_from_settings.as_deref() {
    ...
    if let Some(home) = resolved_home_dir.as_deref() {
        // AX Code installer default.
        push_unique(format!("{home}/.ax-code/bin"));
    ...
    for candidate in candidates {
        ...
        if let Some(bin) = ax_code_binary_from_settings.as_deref() {
            let trimmed = bin.trim();
            if !trimmed.is_empty() {
                cmd = cmd.env("AX_CODE_BINARY", trimmed);
            }
        }

        if let Ok(password) = env::var("AX_CODE_SERVER_PASSWORD") {
            let trimmed = password.trim();
            if !trimmed.is_empty() {
                cmd = cmd.env("AX_CODE_SERVER_PASSWORD", trimmed);
            }
        }
...
fn collect_candidates(
    ...
    walker
        ...
        .follow_links(false)
        .filter_entry(|e| {
            // depth == 0 is the root itself; never skip it regardless of its name
            if e.depth() == 0 {
                return true;
            }
...
fn fuzzy_score(query: &str, candidate: &str) -> Option<i32> {
    ...
    // Substring match — highest priority band (mirrors JS +100 base).
    // find() returns a byte offset; convert to char index before indexing chars[].
    if let Some(byte_pos) = c_lower.find(&q_lower) {
        let char_pos = c_lower[..byte_pos].chars().count();
        let chars: Vec<char> = c_lower.chars().collect();
        let bonus = if char_pos == 0 {
            20
        } else if matches!(chars[char_pos - 1], '/' | '_' | '-' | '.' | ' ') {
            15
        } else {
            0
        };
    ...
    let c_chars: Vec<char> = c_lower.chars().collect();
    let mut qi = 0usize;
    let mut score = 0i32;
    ...
    for (ci, ch) in c_chars.iter().enumerate() {
        if qi < q_chars.len() && *ch == q_chars[qi] {
            let pos_bonus: i32 = if ci < 4 { 18 } else { 10 };
            let gap_penalty: i32 = last_match
                .map(|lm| ((ci - lm - 1) as i32 * 2).min(10))
                .unwrap_or(0);
            consecutive = if last_match == Some(ci.saturating_sub(1)) {
                consecutive + 12
            } else {
                0
            };
            score += pos_bonus - gap_penalty + consecutive;
            last_match = Some(ci);
            qi += 1;
        }
    }
...
fn search_files_impl(
    ...
    let candidates = collect_candidates(directory, include_hidden, respect_gitignore);

    let mut scored: Vec<(i32, usize, FileResult)> = candidates
        .into_iter()
    ...
    // Sort: score DESC, length ASC, name ASC — matches JS sort behaviour
...
async fn desktop_search_files(
    ...
    tokio::task::spawn_blocking(move || {
        search_files_impl(&directory, &query, limit, include_hidden, respect_gitignore)
            .map_err(|e| e.to_string())
    })
    .await
...
mod search_tests {
    ...
    #[test]
    fn substring_match_scores_at_least_100() {
        assert!(fuzzy_score("main", "main.rs").unwrap() >= 100);
        assert!(fuzzy_score("auth", "auth.ts").unwrap() >= 100);
    }

    #[test]
    fn no_match_returns_none() {
        assert_eq!(fuzzy_score("xyz", "main.rs"), None);
        assert_eq!(fuzzy_score("abc", "def.ts"), None);
    }
    ...
    #[test]
    fn word_boundary_bonus_applied() {
        // "auth" after "/" scores higher than "auth" mid-word
        let boundary = fuzzy_score("auth", "middleware/auth.ts").unwrap();
        let mid = fuzzy_score("auth", "middleware/notauth.ts").unwrap_or(0);
        assert!(boundary > mid, "boundary={boundary} mid={mid}");
    }

    #[test]
    fn substring_at_start_scores_120() {
    ...
    #[test]
    fn boundary_bonus_correct_after_multibyte_char() {
        // "é" is 2 bytes; find() returns a byte offset.
        // chars[char_pos - 1] must index by char, not byte, to find '/'.
        let score_boundary = fuzzy_score("auth", "café/auth.ts");
        let score_mid = fuzzy_score("auth", "cafénotauth.ts");
        // boundary ('/' before auth) should outscore mid-word match
        assert!(
            score_boundary.unwrap_or(0) > score_mid.unwrap_or(0),
            "boundary={:?} mid={:?}",
            score_boundary,
            score_mid
        );
    ...
    let builder = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                // Probe the local server to verify ax-code is actually running.
                // spawn_local_server only confirms the sidecar web server responded
                // HTTP 200 — it does not check whether ax-code CLI is ready.
                let local_available = match wait_for_local_ax_code_ready_with(
                    &local_url,
                    LOCAL_SIDECAR_HEALTH_TIMEOUT,
                    LOCAL_SIDECAR_HEALTH_POLL_INITIAL_INTERVAL,
                    LOCAL_SIDECAR_HEALTH_POLL_MAX_INTERVAL,
                ...
                .await
                {
                    Some(probe) if matches!(probe.status.as_str(), "ok" | "auth") => {
                        log::info!("[desktop] local ax-code verified (status={})", probe.status);
                        true
                    }
                    Some(probe) => {
                        log::warn!(
                            "[desktop] local server up but ax-code not ready (status={}), treating as unavailable",
                            probe.status
                        );
                        false
                    }
                    None => {
                        log::warn!("[desktop] local ax-code probe failed, treating as unavailable");
                        false
                    }
                };
...
mod tests {
    ...
    async fn probe_returns_ok_for_valid_openchamber_health_payload() {
        let url = spawn_test_http_server(200, r#"{"axCodeRunning":true}"#).await;
        tokio::time::sleep(std::time::Duration::from_millis(50)).await;
        ...
        tokio::spawn({
            ...
            async move {
                loop {
                    ...
                    let body = if count == 0 {
                        r#"{"status":"ok","axCodeRunning":false,"isAxCodeReady":false}"#
                    } else {
                        r#"{"status":"ok","axCodeRunning":true,"isAxCodeReady":true}"#
                    };
    ...
    #[tokio::test]
    async fn wait_for_local_ax_code_ready_retries_until_health_payload_is_ready() {
        let url = spawn_flaky_openchamber_health_server().await;
        tokio::time::sleep(std::time::Duration::from_millis(50)).await;

        let result = wait_for_local_ax_code_ready_with(
            &url,
    ...
    #[tokio::test]
    async fn wait_for_local_ax_code_ready_returns_last_probe_when_server_never_becomes_ready() {
