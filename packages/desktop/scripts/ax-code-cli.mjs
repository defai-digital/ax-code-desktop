...
const stateFile = path.join(desktopDir, '.ax-code-cli-state.json');
const DEFAULT_BIN_CANDIDATES = [
  process.env.AX_CODE_PATH,
  process.env.AX_CODE_BIN,
  process.env.AX_CODE_BINARY,
  '/opt/homebrew/bin/ax-code',
  '/usr/local/bin/ax-code',
  '/usr/bin/ax-code',
  path.join(os.homedir(), '.local/bin/ax-code'),
].filter(Boolean);
const CLI_ARGS_ENV = process.env.AX_CODE_ARGS;
...
async function resolveCliPath() {
  ...
  for (const segment of envPath.split(path.delimiter)) {
    const candidate = path.join(segment, 'ax-code');
    if (await fileExists(candidate)) {
      return candidate;
    }
  }

  throw new Error('Unable to locate the AX Code CLI. Set AX_CODE_PATH to the executable.');
}
...
export async function startCli({ silent = false } = {}) {
  ...
  if (existing?.pid && isProcessAlive(existing.pid)) {
    if (!silent) {
      console.log(`[desktop:start-cli] AX Code CLI already running (pid ${existing.pid}).`);
    }
    return existing.pid;
  }
  ...
  if (!silent) {
    console.log(`[desktop:start-cli] AX Code CLI started (${cliPath}) pid ${child.pid}.`);
  }
  return child.pid;
}
...
export async function stopCli({ silent = false } = {}) {
  ...
  if (!state?.pid) {
    if (!silent) {
      console.log('[desktop:stop-cli] No AX Code CLI PID recorded.');
    }
    return;
  }
  ...
  while (Date.now() - start < timeoutMs) {
    if (!isProcessAlive(pid)) {
      await removeStateFile();
      if (!silent) {
        console.log('[desktop:stop-cli] AX Code CLI stopped.');
      }
      return;
    }
...
async function main() {
  ...
  if (!command || command === '--help' || command === '-h') {
    console.log('Usage: node ax-code-cli.mjs <start|stop|status>');
    process.exit(0);
  }
  ...
  if (command === 'status') {
    ...
    if (state?.pid && isProcessAlive(state.pid)) {
      console.log(`AX Code CLI running (pid ${state.pid}).`);
    } else {
      console.log('AX Code CLI not running.');
    }
    process.exit(0);
    return;
  }
...
if (import.meta.url === pathToFileURL(process.argv[1] || '').href) {
  main().catch((error) => {
    console.error('[desktop:ax-code-cli] Unexpected error:', error);
    process.exit(1);
  });
}
