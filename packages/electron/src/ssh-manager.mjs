...
export class ElectronSshManager {
  ...
  async spawnMasterProcess(parsed, controlPath, askpassPath, sshPassword) {
    const child = spawn('ssh', buildSshArgs(parsed, [
      ...
      env: {
        ...
        DISPLAY: '1',
        ...(sshPassword ? { AX_CODE_DESKTOP_SSH_ASKPASS_VALUE: sshPassword.trim() } : {}),
      },
    });
    return child;
  }
  ...
  async currentRemoteOpenChamberVersion(parsed, controlPath) {
    try {
      const output = await runRemoteCommand(parsed, controlPath, 'ax-code-desktop --version 2>/dev/null || openchamber --version 2>/dev/null || true');
  ...
  async startRemoteServerManaged(parsed, controlPath, instance, desiredPort) {
    let envPrefix = 'AX_CODE_DESKTOP_RUNTIME=ssh-remote';
    const secret = this.configuredOpenChamberPassword(instance);
    if (secret) {
      envPrefix += ` AX_CODE_DESKTOP_UI_PASSWORD=${shellQuote(secret)}`;
    }
    const output = await runRemoteCommand(parsed, controlPath, `${envPrefix} ax-code-desktop serve --daemon --hostname 127.0.0.1 --port ${desiredPort} || ${envPrefix} openchamber serve --daemon --hostname 127.0.0.1 --port ${desiredPort}`);
