...
let execImpl: (command: string, cwd: string) => WorktreeStatusExecResult | Promise<WorktreeStatusExecResult> = () => ({ command: '', success: false });
...
const revParse = (absoluteGitDir: string, commonDir: string): WorktreeStatusExecResult => ({
...
describe('worktreeStatus.getRootBranch', () => {
  ...
  test('invalidation mid-flight does not let a stale resolve re-seed the cache', async () => {
    let releaseExec: (result: WorktreeStatusExecResult) => void = () => {};
    execImpl = () =>
      new Promise<WorktreeStatusExecResult>((resolve) => {
        releaseExec = resolve;
      });
    statusImpl = () => ({ current: 'main' });
