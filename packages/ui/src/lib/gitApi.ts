...
import { axCodeClient } from './ax-code/client';
...
const resolveSessionGenerationContext = (): SessionGenerationContext | null => {
  ...
  const agent = selection.getSessionAgentSelection(sessionId) || config.currentAgentName || undefined;
...
const runStructuredGenerationInActiveSession = async ({
  ...
  const response = await axCodeClient.withDirectory(directory, async () => {
    return axCodeClient.getApiClient().session.prompt({
      sessionID: generationSession.sessionId,
