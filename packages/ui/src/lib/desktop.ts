...
export type DesktopSettings = {
  ...
  // Optional absolute path to `ax-code` binary.
  axCodeBinary?: string;
...
/**
 * Calls the desktop_search_files Tauri command.
 * Returns null when not running in Tauri (caller must fall back to HTTP).
 */
export const searchFilesNative = async (
  directory: string,
  query: string,
  options?: NativeSearchOptions,
): Promise<NativeFileSearchResult[] | null> => {
  if (!isTauriShell()) return null;
  try {
    const result = await invokeDesktop<NativeFileSearchResult[]>('desktop_search_files', {
      directory,
      query,
