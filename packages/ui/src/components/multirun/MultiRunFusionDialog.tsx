...
const getLastAssistantText = async (source: FusionSource): Promise<string> => {
  ...
  if (messages.length === 0 && source.directory) {
    const result = await axCodeClient.withDirectory(source.directory, () =>
      axCodeClient.getSdkClient().session.messages({
...
export function MultiRunFusionDialog({
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      await axCodeClient.withDirectory(directory ?? axCodeClient.getDirectory(), () =>
        axCodeClient.sendMessage({
          id: fusionSession.id,
          providerID,
          modelID,
