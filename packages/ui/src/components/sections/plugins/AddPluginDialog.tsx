...
type AddPluginTabKey = 'npm' | 'path' | 'file';
...
export const AddPluginDialog: React.FC<AddPluginDialogProps> = ({
  ...
  const [tab, setTab] = React.useState<AddPluginTabKey>('npm');
  ...
  const handleTabChange = (next: AddPluginTabKey) => {
    if (next === tab) return;
    setTab(next);
    resetForm();
  };
    ...
    >
      <DialogContent className="max-w-xl">
        ...
        <SortableTabsStrip
          items={tabs}
          activeId={tab}
          onSelect={(id) => handleTabChange(id as AddPluginTabKey)}
          layoutMode="fit"
          variant="active-pill"
