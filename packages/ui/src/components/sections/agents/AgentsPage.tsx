...
import {
  ...
  type PermissionConfigValue,
  type PermissionRule,
  type PermissionRuleKey,
...
const buildRuleKey = (permission: string, pattern: string): PermissionRuleKey =>
  buildPermissionRuleKey(permission, pattern);

const normalizeRuleset = normalizePermissionRules;
...
export const AgentsPage: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const fetchToolIds = async () => {
      const ids = await axCodeClient.listToolIds({ directory: currentDirectory });
      if (cancelled) {
        return;
      }
