...
import { toMultiRunFileAttachment, type CreateMultiRunParams, type MultiRunFileAttachment, type MultiRunLocalFileAttachment } from '@/types/multirun';
...
export const AgentManagerEmptyState: React.FC<AgentManagerEmptyStateProps> = ({ 
  ...
  const [attachedFiles, setAttachedFiles] = React.useState<MultiRunLocalFileAttachment[]>([]);
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const newFile: MultiRunLocalFileAttachment = {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const files: MultiRunFileAttachment[] | undefined = attachedFiles.length > 0
        ? attachedFiles.map(toMultiRunFileAttachment)
        : undefined;
