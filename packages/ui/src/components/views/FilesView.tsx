...
import { buildCodeMirrorCommentWidgets, normalizeLineRange, useInlineCommentController, type LineRange } from '@/components/comments';
...
interface FilesViewFileRowProps {
  node: FileNode;
  root: string;
  isExpanded: boolean;
  isActive: boolean;
...
const FileRow: React.FC<FilesViewFileRowProps> = ({
  node,
  root,
  isExpanded,
  isActive,
...
export const FilesView: React.FC<FilesViewProps> = ({ mode = 'full' }) => {
  ...
  const extractSelectedCode = React.useCallback((content: string, range: LineRange): string => {
  ...
  const fileCommentController = useInlineCommentController<LineRange>({
    source: 'file',
  ...
  const loadDirectory = React.useCallback(async (dirPath: string) => {
    ...
    const listPromise = files.listDirectory
      ...
      : axCodeClient.listLocalDirectory(normalizedDir, { respectGitignore }).then((result) => result.map((entry) => ({
        name: entry.name,
        path: entry.path,
        isDirectory: entry.isDirectory,
      })));
  ...
  const getFileStatus = React.useCallback((path: string): FileStatus | null => {
    return getFileStatusForPath(path, {
      root,
      isOpen: (filePath) => openPaths.includes(filePath),
      gitStatus,
    });
