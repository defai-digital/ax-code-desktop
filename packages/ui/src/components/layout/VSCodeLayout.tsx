  ...
  React.useEffect(() => {
    const runBootstrap = async () => {
      ...
      try {
        ...
        // If AX Code is still warming up, the initial provider/agent loads can fail and be swallowed by retries.
...
const VSCodeHeader: React.FC<VSCodeHeaderProps> = ({ title, showBack, onBack, onNewSession, onSettings, onAgentManager, showMcp, showContextUsage, showRateLimits, enableSessionSwitcher }) => {
  ...
  const headerMessageSummary = React.useMemo(() => {
    ...
    const latestTokenUsage = findLatestAssistantTokenUsage(currentSessionMessages);
    ...
    return { latestAssistantModel, latestTokenUsage };
  ...
  const contextUsage = React.useMemo<SessionContextUsage | null>(() => {
    if (!currentSessionId || !headerMessageSummary.latestTokenUsage) {
      return null;
    }

    return buildSessionContextUsage(headerMessageSummary.latestTokenUsage.tokens, {
      contextLimit,
      outputLimit,
      lastMessageId: headerMessageSummary.latestTokenUsage.messageId,
    });
  }, [contextLimit, currentSessionId, headerMessageSummary.latestTokenUsage, outputLimit]);
