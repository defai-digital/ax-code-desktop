...
const ChatMessage: React.FC<ChatMessageProps> = ({
    ...
    // Keep non-active-turn rows detached from selection-store churn.
    const currentSessionAgent = useSelectionStore(
        (state) => (isInActiveTurn && sessionId ? state.sessionAgentSelections.get(sessionId) : undefined)
    );
    ...
    const agentName = React.useMemo(() => {
        ...
        return currentSessionAgent ?? undefined;
    }, [isUser, message.info, previousIsModeSwitchMessage, previousUserMetadata, sessionId, currentSessionAgent]);
                        ...
                        <div className="relative">
                            {shouldShowHeader && (
                                <MessageHeader
                                    ...
                                    timestamp={messageCreatedAt ?? undefined}
                                />
                            )}
