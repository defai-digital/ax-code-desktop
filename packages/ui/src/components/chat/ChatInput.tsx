...
type SessionSendMessage = ReturnType<typeof useSessionUIStore.getState>['sendMessage'];
...
const ChatInputComponent: React.FC<ChatInputProps> = ({ onOpenSettings, scrollToBottom }) => {
    ...
    const sendMessage = React.useRef<SessionSendMessage>((...args) =>
                        ...
                        for (const path of paths) {
                            try {
                                ...
                                if (isDesktopShell()) {
                                    ...
                                    const response = await fetch(`${API_ENDPOINTS.fs.raw}?path=${encodeURIComponent(normalizedPath)}`);
                                    if (!response.ok) {
                    ...
                    {!projectKnowledge.isLoading && (
                        <div className="mt-3 flex items-center justify-center gap-2">
                            {projectKnowledge.exists ? (
                                ...
                                <button
                                    type="button"
                                    onClick={() => useInputStore.getState().requestPresetSubmit(t('chat.projectKnowledge.createPrompt'))}
