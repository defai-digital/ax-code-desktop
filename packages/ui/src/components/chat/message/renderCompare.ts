...
export const areRenderRelevantMessagesEqual = (left: SessionMessageRecord, right: SessionMessageRecord): boolean => {
  return areRenderRelevantMessageInfoEqual(left.info, right.info) && areRenderRelevantPartsEqual(left.parts, right.parts);
};

export const areOptionalRenderRelevantMessagesEqual = (left?: SessionMessageRecord, right?: SessionMessageRecord): boolean => {
  if (!left || !right) {
    return left === right;
  }
