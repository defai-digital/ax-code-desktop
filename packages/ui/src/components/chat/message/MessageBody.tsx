...
const UserMessageBody = React.memo(({ messageId, parts, isMobile, alwaysShowActions = isMobile, hasTouchInput, hasTextContent, onCopyMessage, copiedMessage, onShowPopup, agentMention, onRevert, onFork, userActionsMode = 'inline', stickyUserHeaderEnabled = true, messageCreatedAt }: {
    messageId: string;
    parts: Part[];
    isMobile: boolean;
    alwaysShowActions?: boolean;
    ...
    const actionsBlock = ((canCopyMessage && hasCopyableText) || onRevert || effectiveOnFork || showTimestampInActionsRow) && showUserActions ? (
        <div className={cn(
            'group/user-actions',
            isMobile
                ? userActionsMode === 'inline'
                    ? 'flex items-center justify-between pt-2 pb-3'
                    : stickyUserHeaderEnabled
                        ? 'flex h-9 items-center justify-between pt-0'
                        : 'flex h-11 items-center justify-between pt-0'
                : userActionsMode === 'inline'
                    ? 'absolute top-full left-0 right-0 z-10 pt-5 flex items-center justify-between'
                    : 'flex h-8 items-center justify-between pt-2'
        )}>
