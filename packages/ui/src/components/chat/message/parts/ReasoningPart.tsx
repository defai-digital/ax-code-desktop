...
export const MergedReasoningPart = React.memo(({
    ...
    const mergedText = React.useMemo(() => {
        return parts
            .map((part) => {
                return cleanReasoningText(extractTextOrContent(part));
            })
            .filter((t) => t.length > 0)
            .join('\n\n');
    }, [parts]);
    ...
    const mergedTime = React.useMemo(() => {
        ...
        for (const part of parts) {
            const time = (part as TimedTextContentPart).time;
    ...
    const isStreaming = chatRenderMode === 'live' && canBeStreaming && parts.some(
        (part) => typeof (part as TimedTextContentPart).time?.end !== 'number',
    );
