...
const DiffPreview: React.FC<ToolDiffPreviewProps> = React.memo(({ diff, pierreTheme, pierreThemeType, diffViewMode }) => {
            ...
            try {
                ...
                if (Array.isArray(messages) && messages.length > 0) {
                    ...
                    childStores.update(currentDirectory, (prev) => {
                        ...
                        const partPatch: Record<string, import('@ax-code/sdk/v2').Part[]> = { ...prev.part };
                        for (const rec of records) {
                            partPatch[rec.info.id] = rec.parts;
                        }
                        return {
                            message: { ...prev.message, [capturedSessionId]: records.map((r) => r.info) as import('@ax-code/sdk/v2').Message[] },
                            part: partPatch,
                        };
                    });
                }
        ...
        const fetchSessionMessages = async (isInitialFetch: boolean) => {
            try {
                ...
                childStores.update(currentDirectory, (prev) => {
                    ...
                    return {
                        message: { ...prev.message, [taskSessionId]: records.map((r) => r.info) as import('@ax-code/sdk/v2').Message[] },
                        part: partPatch,
                    };
                });
            } catch {
