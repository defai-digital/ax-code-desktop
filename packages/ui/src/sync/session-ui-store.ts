...
export function routeMessage(params: {
  ...
  if (params.directory !== undefined) {
    return axCodeClient.withDirectory(params.directory, run)
  }
...
export const useSessionUIStore = create<SessionUIState>()((set, get) => ({
  ...
  setCurrentSession: (id, directoryHint?: string | null) => {
    ...
    try {
      ...
      axCodeClient.setDirectory(resolvedDir ?? undefined)
    } catch (e) {
      console.warn("Failed to set ax-code directory for session switch:", e)
    }
  ...
  createSession: async (title, directoryOverride, parentID) => {
    ...
    try {
      const dir = directoryOverride ?? axCodeClient.getDirectory()
