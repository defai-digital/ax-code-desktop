...
export async function bootstrapGlobal(
    ...
    retry(() =>
      sdk.project.list().then((x) => {
        ...
        const projects = data
          ...
          .filter((p) => !!p.worktree && !p.worktree.includes("ax-code-test"))
  ...
  // If ALL requests failed, AX Code is likely down — fetch the OpenChamber
    ...
    try {
      ...
      if (healthRes.ok) {
        ...
        if (health.lastAxCodeError) {
          message = health.lastAxCodeError
        } else if (!health.axCodeRunning) {
          message = "ax-code process is not running"
        }
      }
    } catch {
