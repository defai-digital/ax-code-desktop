...
export function ElectronMiniChatApp({ apis }: ElectronMiniChatAppProps) {
  ...
  React.useEffect(() => {
    axCodeClient.setDirectory(currentDirectory || config.directory || undefined);
  ...
  return (
    <ErrorBoundary>
      <SyncProvider sdk={axCodeClient.getSdkClient()} directory={currentDirectory || config.directory || ''}>
