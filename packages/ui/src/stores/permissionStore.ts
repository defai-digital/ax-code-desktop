        ...
        persist(
            (set, get) => ({
                ...
                setSessionAutoAccept: async (sessionId: string, enabled: boolean) => {
                    ...
                    // ancestry lookup can lag AX Code session indexing.
                    for (const scopedSessionId of sessionScope) {
                        void fetch(API_ENDPOINTS.notifications.autoAccept, {
                            method: 'POST',
                    ...
                    const currentDirectory = normalizeDirectoryCandidate(axCodeClient.getDirectory());
                    if (currentDirectory) {
                    ...
                    const pendingFromApi = await axCodeClient
