...
export const useChatAutoFollow = ({
    ...
    const unseenBaseRef = React.useRef(sessionMessageCount);
    ...
    const releaseAutoFollow = React.useCallback(() => {
        stopFollowLoop();
        stopSettleBurst();
        unseenBaseRef.current = sessionMessageCountRef.current;
    ...
    const unseenCount = state === 'released'
        ? Math.max(0, sessionMessageCount - unseenBaseRef.current)
        : 0;
