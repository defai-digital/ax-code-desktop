...
- `packages/web/server/lib/skills-catalog/`: Skills catalog module directory containing all skill-related functionality.
  ...
  - `shared.js`: Shared validation, filesystem, and install target path helpers used by scan/install providers.
...
- `toRepoFsPath(repoDir, repoRelPosixPath)`: Convert repository POSIX paths to local filesystem paths.
- `normalizeUserSkillDir(userSkillDir)`: Normalize user skill directory path (handles legacy `~/.config/ax-code/skill` → `~/.config/ax-code/skills` migration).
