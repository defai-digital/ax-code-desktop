...
export async function cloneRepo({ cloneUrl, identity, tempDir, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  ...
  const result = await runGit(preferred, { identity, timeoutMs });
  if (result.ok) return { ok: true };

  const fallbackResult = await runGit(fallback, { identity, timeoutMs });
