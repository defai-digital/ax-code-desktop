...
const AX_CODE_WORKTREE_ATTEMPTS = 26;

const getAxCodeDataPath = () => {
  const xdgDataHome = process.env.XDG_DATA_HOME || path.join(os.homedir(), '.local', 'share');
  return path.join(xdgDataHome, 'ax-code');
};

const pickRandom = (values) => values[Math.floor(Math.random() * values.length)];

const generateAxCodeRandomName = () => `${pickRandom(AX_CODE_ADJECTIVES)}-${pickRandom(AX_CODE_NOUNS)}`;
...
const ensureAxCodeProjectId = async (primaryWorktree) => {
  const gitDir = path.join(primaryWorktree, '.git');
  const idFile = path.join(gitDir, 'ax-code');
  ...
  if (!projectId) {
    throw new Error('Failed to derive AX Code project ID');
  }
...
const resolveWorktreeProjectContext = async (directory) => {
  ...
  const projectID = await ensureAxCodeProjectId(primaryWorktree);
  const worktreeRoot = path.join(getAxCodeDataPath(), 'worktree', projectID);
...
const resolveWorktreeNameCandidates = (baseName) => {
  ...
  if (!normalizedBase) {
    return Array.from({ length: AX_CODE_WORKTREE_ATTEMPTS }, () => generateAxCodeRandomName());
  }
  return Array.from({ length: AX_CODE_WORKTREE_ATTEMPTS }, (_, index) => {
    if (index === 0) {
      return normalizedBase;
    }
    return `${normalizedBase}-${generateAxCodeRandomName()}`;
  });
};
...
const loadProjectStartCommand = async (projectID) => {
  const storagePath = path.join(getAxCodeDataPath(), 'storage', 'project', `${projectID}.json`);
  try {
...
export async function createWorktree(directory, input = {}) {
  ...
  try {
    ...
    console.warn('Failed to sync AX Code sandbox metadata (add):', error instanceof Error ? error.message : String(error));
  }
...
export async function removeWorktree(directory, input = {}) {
  ...
  if (!matchedEntry?.worktree) {
    ...
    try {
      ...
      console.warn('Failed to sync AX Code sandbox metadata (remove):', error instanceof Error ? error.message : String(error));
    }
