...
Each provider module registered in `providers/index.js` should export:
- `providerId`: stable exact dispatch ID.
- `providerName`: display name for responses.
- `aliases`: auth/config lookup aliases.
- `isConfigured()`: whether quota can be fetched with current configuration.
- `fetchQuota()`: returns the shared quota response shape described above.
