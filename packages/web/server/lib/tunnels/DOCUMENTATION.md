...
## Purpose
This module contains tunnel provider orchestration for AX Code Desktop, including provider registry/service wiring, managed remote token config lifecycle, and tunnel HTTP route registration.
...
- `packages/web/server/lib/tunnel-binaries.js`: shared PATH binary resolution for tunnel provider CLIs.
