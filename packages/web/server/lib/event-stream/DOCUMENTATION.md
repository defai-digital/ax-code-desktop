...
- `packages/web/server/lib/event-stream/upstream-health.js`: shared initial-upstream health-check trigger policy used by WS bridges.
...
- Global synthetic events such as `openchamber:session-status`, `openchamber:session-activity`, `openchamber:notification`, and `openchamber:heartbeat` are preserved on the WS path for compatibility, but heartbeat frames are emitted only while an upstream SSE stream is actively attached.
- Global UI broadcasts are fan-out capable across both SSE and WS clients.
- The reusable upstream reader centralizes SSE fetch/parsing/reconnect behavior for the WS runtime and AX Code watcher. Additional event consumers should move to it only with parity tests for their lifecycle and error semantics.
