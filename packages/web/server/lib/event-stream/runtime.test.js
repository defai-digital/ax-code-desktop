...
describe('message stream websocket runtime', () => {
  ...
  it('closes the websocket and triggers health check on initial upstream unavailable response', async () => {
    ...
    expect(socket.sent).toEqual([
      {
        type: 'error',
        message: 'AX Code event stream unavailable (503)',
      },
    ]);
    expect(socket.closeCalls).toEqual([
      {
        code: 1011,
        reason: 'AX Code event stream unavailable',
      },
    ]);
  ...
  it('closes the websocket without health check when AX Code URL cannot be built', async () => {
    const server = new EventEmitter();
    const wsClients = new Set();
    let triggerHealthCheckCalls = 0;
    let fetchCalls = 0;
    ...
    const runtime = createMessageStreamWsRuntime({
      ...
      buildAxCodeUrl() {
        throw new Error('missing AX Code port');
      },
      getAxCodeAuthHeaders: () => ({}),
      processForwardedEventPayload() {},
      wsClients,
    ...
    expect(socket.sent).toEqual([
      {
        type: 'error',
        message: 'AX Code service unavailable',
      },
    ]);
    expect(socket.closeCalls).toEqual([
      {
        code: 1011,
        reason: 'AX Code service unavailable',
      },
    ]);
