...
export const registerSkillRoutes = (app, dependencies) => {
  ...
  const inferSkillScopeAndSourceFromPath = (skillPath, workingDirectory) => {
    ...
    const userRoots = [
      path.join(home, '.config', 'ax-code'),
      path.join(home, '.ax-code'),
      path.join(home, '.claude', 'skills'),
      path.join(home, '.agents', 'skills'),
      process.env.AX_CODE_CONFIG_DIR ? path.resolve(process.env.AX_CODE_CONFIG_DIR) : null,
    ].filter(Boolean);
  ...
  const fetchAxCodeDiscoveredSkills = async (workingDirectory) => {
    if (!getAxCodePort()) {
      return [];
    }
    ...
    try {
      const client = createAxCodeClient({
        baseUrl: buildAxCodeUrl('/', '').replace(/\/$/, ''),
        directory: workingDirectory || undefined,
        headers: getAxCodeAuthHeaders(),
      ...
      console.error('Failed to list ax-code skills:', error);
      return [];
    }
  };
  ...
  app.get('/api/config/skills', async (req, res) => {
    try {
      ...
      const axCodeSkills = await fetchAxCodeDiscoveredSkills(directory);
      const localSkills = discoverSkills(directory);
      const skills = mergeDiscoveredSkills(axCodeSkills, localSkills);
  ...
  app.get('/api/config/skills/catalog/source', async (req, res) => {
    try {
      ...
      const resolvedDiscovered = mergeDiscoveredSkills(
        await fetchAxCodeDiscoveredSkills(directory),
  ...
  app.post('/api/config/skills/install', async (req, res) => {
    try {
      ...
      if (isClawdHubSource(source)) {
        ...
        if (requiresReload) {
          await refreshAxCodeAfterConfigChange('skills install');
        }
  ...
  app.get('/api/config/skills/:name', async (req, res) => {
    try {
      ...
      const discoveredSkill = (await fetchAxCodeDiscoveredSkills(directory))
  ...
  app.delete('/api/config/skills/:name', async (req, res) => {
    try {
      ...
      await refreshAxCodeAfterConfigChange('skill deletion');
