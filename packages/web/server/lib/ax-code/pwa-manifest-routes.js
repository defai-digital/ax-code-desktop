const DEFAULT_PWA_APP_NAME = 'AX Code - AI Coding Assistant';
...
export const registerPwaManifestRoute = (app, dependencies) => {
  ...
  const getRecentPwaSessionShortcuts = async (req) => {
    ...
    const listSessions = async (directory) => {
      ...
      const response = await fetch(buildAxCodeUrl(`/session${query}`, ''), {
        method: 'GET',
        headers: {
    ...
    try {
      ...
      for (const [key, entry] of recentPwaSessionsCache) {
        if (now - entry.at >= 5000) {
          recentPwaSessionsCache.delete(key);
        }
      }
  ...
  app.get('/manifest.webmanifest', async (req, res) => {
    ...
    const manifest = {
      ...
      description: 'Web interface companion for ax-code AI coding agent',
      id: '/',
      start_url: '/',
      scope: '/',
      display: 'standalone',
