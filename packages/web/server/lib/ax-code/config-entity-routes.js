export const registerConfigEntityRoutes = (app, dependencies) => {
  ...
  const completeMcpMutation = async (res, action, name, applyChange) => {
    ...
    try {
      await refreshAxCodeAfterConfigChange(`mcp ${action}`);
      return res.json({
        success: true,
        requiresReload: true,
      ...
      return res.json({
        ...
        reloadFailed: true,
        message: `MCP server "${name}" ${action}d, but ax-code reload failed.`,
        warning: error.message || 'ax-code reload failed after the MCP configuration changed',
      });
    }
  };
  ...
  app.delete('/api/config/agents/:name', async (req, res) => {
    try {
      ...
      await refreshAxCodeAfterConfigChange('agent deletion');
  ...
  app.delete('/api/config/commands/:name', async (req, res) => {
    try {
      ...
      await refreshAxCodeAfterConfigChange('command deletion');
