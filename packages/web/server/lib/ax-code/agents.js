...
function ensureProjectAgentDir(workingDirectory) {
  return ensureProjectAxCodeResourceDirs(workingDirectory, 'agents', 'agent');
}
...
function getUserAgentPath(agentName, lookupCache = null) {
  ...
  const pluralPath = resolveUserAxCodeResourcePath(
    ['agents', `${agentName}.md`],
    ['agent', `${agentName}.md`],
  );
...
function getAgentSources(agentName, workingDirectory, lookupCache = createAgentLookupCache()) {
  return getConfigEntitySources({
    ...
    bodyField: 'prompt',
    scopes: AGENT_SCOPE,
    getProjectPath: getProjectAgentPath,
    getUserPath: (name) => getUserAgentPath(name, lookupCache),
  });
}
...
function createAgent(agentName, config, workingDirectory, scope) {
  ...
  if (jsonSource.exists) {
    throw new Error(`Agent ${agentName} already exists in ax-code.json`);
  }
...
function deleteAgent(agentName, workingDirectory) {
  ...
  if (jsonSource.exists && jsonSource.config && jsonSource.path) {
    ...
    console.log(`Removed agent from ax-code.json: ${agentName}`);
    deleted = true;
  }
