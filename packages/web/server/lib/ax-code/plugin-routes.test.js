...
let rootDir;
let plugins;
let refreshAxCodeAfterConfigChange;
let app;
let cleanupPaths;
...
describe('ax-code plugin routes', () => {
  beforeAll(async () => {
    rootDir = fs.mkdtempSync(path.join(os.tmpdir(), 'openchamber-plugin-routes-'));
    userConfigPath = path.join(rootDir, 'user-ax-code.json');
  ...
  test('GET /registry with empty specs returns empty results', async () => {
    const getNpmInfo = vi.fn(async () => ({ ok: true, latest: '1.0.0', versions: ['1.0.0'], distTags: { latest: '1.0.0' } }));
  ...
  test('GET /registry reports update for exact npm version behind latest', async () => {
    createRegistryApp(vi.fn(async () => ({ ok: true, latest: '2.0.0', versions: ['1.0.0', '2.0.0'], distTags: { latest: '2.0.0' } })));
  ...
  test('GET /registry reports no update when exact npm version matches latest', async () => {
    createRegistryApp(vi.fn(async () => ({ ok: true, latest: '1.0.0', versions: ['1.0.0'], distTags: { latest: '1.0.0' } })));
  ...
  test('GET /registry reports missing npm package', async () => {
    createRegistryApp(vi.fn(async () => ({ ok: false, status: 404, error: 'Package not found' })));
  ...
  test('GET /registry reports npm network failure without failing route', async () => {
    createRegistryApp(vi.fn(async () => ({ ok: false, status: 'network', error: 'socket closed' })));
  ...
  test('GET /registry deduplicates npm package lookups by name', async () => {
    const getNpmInfo = vi.fn(async () => ({ ok: true, latest: '3.0.0', versions: ['1', '2', '3'], distTags: { latest: '3.0.0' } }));
    createRegistryApp(getNpmInfo);
  ...
  test('POST /entry creates entry and requires reload', async () => {
    ...
    expect(refreshAxCodeAfterConfigChange).toHaveBeenCalledWith('plugin entry creation');
  });
  ...
  test('PATCH /entry/:id updates entry in same array index', async () => {
    ...
    expect(refreshAxCodeAfterConfigChange).toHaveBeenCalledWith('plugin entry update');
  });
  ...
  test('DELETE /entry/:id removes entry and prunes plugin key', async () => {
    ...
    expect(refreshAxCodeAfterConfigChange).toHaveBeenCalledWith('plugin entry deletion');
  });
  ...
  test('POST /file writes plugin dir file', async () => {
    ...
    expect(refreshAxCodeAfterConfigChange).toHaveBeenCalledWith('plugin file creation');
  });
  ...
  test('PUT /file/:id updates file content', async () => {
    ...
    expect(refreshAxCodeAfterConfigChange).toHaveBeenCalledWith('plugin file update');
  });
  ...
  test('DELETE /file/:id unlinks file', async () => {
    ...
    expect(refreshAxCodeAfterConfigChange).toHaveBeenCalledWith('plugin file deletion');
  });
