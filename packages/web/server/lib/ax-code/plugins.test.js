...
import { readJson, writeJson } from './test-helpers.js';
...
describe('ax-code plugins data layer', () => {
  beforeAll(async () => {
  ...
  test('routes project entries to project config and user entries to custom user config', () => {
    ...
    expect(readJson(path.join(projectDir, '.ax-code', 'ax-code.json')).plugin).toEqual(['project-plugin']);
  });

  test('re-resolves custom config env between calls', () => {
    const firstConfigPath = path.join(rootDir, 'first', 'ax-code.json');
    const secondConfigPath = path.join(rootDir, 'second', 'ax-code.json');

    process.env.AX_CODE_CONFIG = firstConfigPath;
    ...
    process.env.AX_CODE_CONFIG = secondConfigPath;
  ...
  test('lists entries from user and project layers with scopes and parsed kinds', () => {
    ...
    writeJson(path.join(projectDir, '.ax-code', 'ax-code.json'), { plugin: ['./local-plugin.js'] });
    ...
    expect(entries).toEqual([
      ...
      expect.objectContaining({ spec: './local-plugin.js', scope: 'project', kind: 'config', parsedKind: 'path', sourcePath: path.join(projectDir, '.ax-code', 'ax-code.json') }),
    ]);
    fs.rmSync(userConfigPath, { force: true });
  ...
  test('rejects duplicate plugin dir files unless overwrite is true', () => {
    ...
    expect(fs.readFileSync(path.join(projectDir, '.ax-code', 'plugins', 'dup.js'), 'utf8')).toBe('two');
  });

  test('lists only valid plugin dir files', () => {
    const dir = path.join(projectDir, '.ax-code', 'plugins');
