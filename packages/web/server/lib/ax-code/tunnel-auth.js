...
export const createTunnelAuth = () => {
  ...
  const clearActiveTunnel = () => {
    if (activeTunnelId) {
      const clearedTunnelId = activeTunnelId;
      revokeTunnelArtifacts(clearedTunnelId);
      for (const [sessionId, record] of tunnelSessions) {
        if (record.tunnelId === clearedTunnelId) {
  ...
  const checkConnectRateLimit = (req) => {
    ...
    if (!record || now - record.lastAttempt > CONNECT_RATE_LIMIT_WINDOW_MS) {
      if (record) connectRateLimiter.delete(key);
