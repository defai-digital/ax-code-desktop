...
export const registerPluginRoutes = (app, dependencies) => {
  ...
  const completePluginMutation = async (res, operation, _noun, applyChange) => {
    ...
    try {
      await refreshAxCodeAfterConfigChange(`plugin ${operation}`);
      ...
      return res.json({
        ...
        message: `Plugin ${pastTense}, but ax-code reload failed.`,
        reloadDelayMs: clientReloadDelayMs,
        reloadFailed: true,
        warning: error.message || 'ax-code reload failed after plugin config changed',
      });
    }
  };
