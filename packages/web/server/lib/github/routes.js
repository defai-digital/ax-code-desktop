...
export function registerGitHubRoutes(app) {
    ...
    try {
      ...
      if (!clientId) {
        return res.status(400).json({
          error: 'GitHub OAuth client not configured. Set AX_CODE_DESKTOP_GITHUB_CLIENT_ID.',
        });
      }
  ...
  app.get('/api/github/pr/status', async (req, res) => {
    try {
      ...
      // Cache successful responses before sending. Only caches connected:true responses.
      const sendJson = (data) => {
      ...
      const octokit = getOctokitOrNull();
      if (!octokit) {
        return sendJson({ connected: false });
      }
      ...
      if (!searchRepo) {
        return sendJson({ connected: true, repo: null, branch, pr: null, checks: null, canMerge: false, defaultBranch: null, resolvedRemoteName: null });
      }
      if (!first) {
        return sendJson({ connected: true, repo: searchRepo, branch, pr: null, checks: null, canMerge: false, defaultBranch: resolvedStatus.defaultBranch ?? null, resolvedRemoteName: resolvedStatus.resolvedRemoteName ?? null });
      }
      ...
      if (!prData) {
        return sendJson({ connected: true, repo: searchRepo, branch, pr: null, checks: null, canMerge: false });
      }
