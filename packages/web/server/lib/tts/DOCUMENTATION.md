...
### `isAvailable()`
Returns boolean indicating whether OpenAI API key is configured (checks environment variable `OPENAI_API_KEY` or AX Code auth file).
...
2. AX Code auth file (`auth.openai`, `auth.codex`, or `auth.chatgpt`).
...
### Manual verification
1. Configure OpenAI API key via environment variable or AX Code settings.
